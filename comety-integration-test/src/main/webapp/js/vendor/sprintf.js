	...
	for (var i = 0; i < format_length; i++) {
		...
		case 3: // while defining precision
			...
			case 'f':
				...
				break;
			case 'e':
				result += toExponential(argv[index], flag, width, precision,
						'e');
				index++;
				mode = 0;
				break;
		...
		if (dec > p) { // 切る
			...
			if (tmp > 5) { // 切り上げ
				...
				if (n < 1) {
					...
					tmp = (parseInt(str.substring(0, pos_dot)
							+ str.substring(pos_dot + 1, pos_dot + p + 1)) + 1)
							.toString();
...
function toExponential(n, f, w, p, e) {
	...
	if (type == 0) { // 整数
		...
		if (n > 10) {
			exp = pos_dot - 1;
			str = str.substring(0, 1) + "." + str.substring(1, pos_dot)
					+ str.substring(pos_dot + 1);
			pos_dot = 1;
		} else if (n > 1)
			exp = 0;
		else {
