describe("CometyServiceTest", function() {
	...
	it("CometyService作成時に設定オブジェクトがない場合はエラー", function () {
		expect(CometyService).to.throwException("options object is required.");
	});
	
	it("CometyService作成時にrootPathの設定がな場合はエラー", function () {
		expect(CometyService).withArgs({}).to.throwException("options.rootPath is requierd.");
	});
	
	it("クローズの呼び出し時にセッションIDがない場合はエラー", function () {
		expect(this.cometyService.close.bind(this.cometyService)).to.throwException("not connect comety service.");
	});
	
	it("ポーリングの呼び出し時にセッションIDがない場合はエラー", function () {
		expect(this.cometyService.polling.bind(this.cometyService)).to.throwException("not connect comety service.");
	});
	
	it("メッセージ送信の呼び出し時にセッションIDがない場合はエラー", function () {
		expect(this.cometyService.sendMessage.bind(this.cometyService)).to.throwException("not connect comety service.");
	});
	...
	it("pollingでオープンメッセージが返却された場合にonOpenが呼び出され、その後pollingを再開することを確認する", function () {
		...
		this.server.respondWith(
			"POST",
			"jaxrs/polling",
			[200, {"Content-Type": "application/json", "comety-status": "open"}, JSON.stringify({"message": ""})]
		);
	...
	it("pollingでクローズメッセージが返却された場合にonCloseが呼び出され、その後sessionIdをnullにすることを確認する", function () {
		...
		this.server.respondWith(
			"POST",
			"jaxrs/polling",
			[200, {"Content-Type": "application/json", "comety-status": "close"}, JSON.stringify({"message": ""})]
		);
	...
	it("pollingでメッセージが返却された場合にonMessageが呼び出され、その後pollingを再開することを確認する", function () {
		...
		this.server.respondWith(
			"POST",
			"jaxrs/polling",
			[200, {"Content-Type": "application/json", "comety-status": "message"}, JSON.stringify({"message": ""})]
		);
	...
	it("pollingでタイムアウトが返却された場合にpollingを再開することを確認する", function () {
		...
		this.server.respondWith(
			"POST",
			"jaxrs/polling",
			[200, {"Content-Type": "application/json", "comety-status": "timeout"}, JSON.stringify({"message": ""})]
		);
	...
	it("pollingで未知のステータスが返却された場合はコンソールに警告を出力する", function () {
		...
		this.server.respondWith(
			"POST",
			"jaxrs/polling",
			[200, {"Content-Type": "application/json", "comety-status": "none"}, JSON.stringify({"message": ""})]
		);
