...
public class CometySession {
     ...
     * セッション格納
     */
    protected static final Map<String, CometySession> sessions = new HashMap<>();

    /**
     * セッションを作成する
     *
     * @param cometService 親サービス
    ...
    /**
     * セッションを取得する 存在しない場合はCometSessionNotFoundExceptionを発生させる
     *
     * @param sessionId セッションID
     ...
     * メッセージをためるキュー
     */
    protected Queue<CometyMessage> queue = new LinkedList<>();
    ...
    public boolean isOpenPolling() {
        return asyncContext != null && !asyncContext.isComplete();
    }
    ...
    public synchronized void startPolling(HttpServletRequest request) {
        CometyAsyncContext newAsyncContext = new CometyAsyncContext(this, request.startAsync());
        newAsyncContext.setTimeout(cometService.getCometyServiceInfo().getTimeout());
        newAsyncContext.addListener(new CometyAsyncListener(this));
